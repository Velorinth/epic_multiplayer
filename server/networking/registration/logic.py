import json
from typing import TYPE_CHECKING, Callable, Dict, Tuple, Any

# These are needed for the handlers
from update.data import entities
from loader.content import yml_content
from networking.registration.handle import handle_join, handle_get_entities, handle_update_player
...
def register_packet(packet_type: str):
    ...
    def decorator(func: Callable):
        ...
        print(f"Registered handler for packet type '{packet_type}'")
        return func
    return decorator
...
def register_subtype(packet_type: str, sub_type: str):
    ...
    def decorator(func: Callable):
        ...
        print(f"Registered sub-handler for packet type '{packet_type}' with subtype '{sub_type}'")
        return func
    return decorator

def packet_handler(server: 'TCPServer', addr: Tuple[str, int], packet_data: Dict[str, Any]):
    """
    ...
    if not packet_type:
        print(f"Received packet from {addr} with no type: {packet_data}")
        return
    ...
    # Fallback to general type handlers if no subtype handler was found
    if not handler:
    ...
    if handler:
        try:
            handler(server, addr, packet_data)
        except Exception as e:
            print(f"Error handling packet {handler_id} from {addr}: {e}")
    else:
        # Refine the "not found" message
        if sub_type:
            handler_id = f"type '{packet_type}' with subtype '{sub_type}'"
        print(f"No handler registered for packet {handler_id} from {addr}")
